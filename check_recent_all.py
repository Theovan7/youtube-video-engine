#!/usr/bin/env python3
"""Check all recent jobs regardless of type."""

from datetime import datetime, timedelta
from services.airtable_service import AirtableService
import json

airtable = AirtableService()

def check_all_recent_jobs():
    """Check all recent jobs."""
    print("Checking All Recent Jobs")
    print("=" * 80)
    
    # Get jobs table
    jobs_table = airtable.jobs_table
    all_jobs = jobs_table.all()
    
    # Filter for recent jobs
    recent_jobs = []
    three_hours_ago = datetime.now() - timedelta(hours=3)
    
    for job in all_jobs:
        fields = job.get('fields', {})
        created_time = fields.get('Created Time', '')
        
        if created_time:
            try:
                created_dt = datetime.fromisoformat(created_time.replace('Z', ''))
                if created_dt > three_hours_ago:
                    recent_jobs.append({
                        'id': job['id'],
                        'created': created_dt,
                        'status': fields.get('Status', 'Unknown'),
                        'type': fields.get('Job Type', 'Unknown'),
                        'external_id': fields.get('External Job ID'),
                        'segment_id': fields.get('Segment ID'),
                        'response': fields.get('Response Payload')
                    })
            except:
                pass
    
    # Sort by creation time
    recent_jobs.sort(key=lambda x: x['created'], reverse=True)
    
    print(f"\nFound {len(recent_jobs)} jobs in the last 3 hours:\n")
    
    # Group by type
    by_type = {}
    for job in recent_jobs:
        job_type = job['type']
        if job_type not in by_type:
            by_type[job_type] = []
        by_type[job_type].append(job)
    
    # Show summary
    print("Summary by type:")
    for job_type, jobs in by_type.items():
        print(f"  {job_type}: {len(jobs)} jobs")
    print()
    
    # Show latest 15 jobs
    for job in recent_jobs[:15]:
        time_ago = datetime.now() - job['created'].replace(tzinfo=None)
        minutes = int(time_ago.total_seconds() / 60)
        
        print(f"Job: {job['id']}")
        print(f"  Type: {job['type']}")
        print(f"  Created: {minutes} minutes ago")
        print(f"  Status: {job['status']}")
        print(f"  Segment: {job['segment_id']}")
        print(f"  External ID: {job['external_id'] or 'None'}")
        
        if job['response']:
            try:
                response = json.loads(job['response']) if isinstance(job['response'], str) else job['response']
                if isinstance(response, dict):
                    if 'error' in response:
                        print(f"  ❌ Error: {response['error']}")
                    elif 'job_id' in response:
                        print(f"  ✓ Job ID: {response['job_id']}")
                    else:
                        print(f"  Response: {str(response)[:100]}")
                else:
                    print(f"  Response: {str(response)[:100]}")
            except:
                print(f"  Response: {job['response'][:100] if job['response'] else 'None'}")
        
        print("-" * 40)

if __name__ == "__main__":
    check_all_recent_jobs()